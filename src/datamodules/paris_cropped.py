"""Lightning DataModule for the Paris cropped dataset."""
...
from ..datasets import ParisCropped


class ParisCroppedDataModule(pl.LightningDataModule):
    """DataModule providing dataloaders for the Paris cropped tiles."""

    mean = torch.tensor([0.2563, 0.2543, 0.2532], dtype=torch.float32)
    std = torch.tensor([0.2218, 0.2168, 0.2151], dtype=torch.float32)
    ...
    def setup(self, stage: Optional[str] = None) -> None:  # noqa: D401
        ...
        splits_available = all(
            (Path(self.root) / split).exists() for split in ParisCropped.splits
        )

        if splits_available:
            self.train_dataset = ParisCropped(
                self.root, split="train", transforms=self.transforms
            )
            self.val_dataset = ParisCropped(
                self.root, split="val", transforms=self.transforms
            )
            self.test_dataset = ParisCropped(
                self.root, split="test", transforms=self.transforms
            )
        else:
            full_dataset = ParisCropped(
    ...
    def _split_lengths(self, dataset_size: int) -> list[int]:
        ...
        if train <= 0:
            raise ValueError(
                "ParisCroppedDataModule requires more samples than reserved for"
